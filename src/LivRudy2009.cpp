...
void LivRudy2009::solve(){
  ...
  Jrelinf = GJrel_ * alpha_rel * beta_tau * ICaL /
      (exp(hrel * log(Krel_inf / CaJSR)) + 1.0);
  tau_rel = beta_tau / (Krel_tau / CaJSR + 1.0);
  ...
  dNai = -(INa + INab + ICaL_Na + 3.0 * INCX + 3.0 * INaK) * Acap / (Vmyo * F);
  // Injected current assumed to be due to potassium flux
  dKi = -(I_Inject + IKr + IKs + IK1 + IKp + ICaL_K - 2.0 * INaK) * Acap /
      (Vmyo * F);
  dCai = (-Jserca * VNSR / Vmyo + Jrel * VJSR / Vmyo -
               (ICaL + ICaT + ICab + IpCa - 2.0 * INCX) *
               Acap / (2.0 * Vmyo * F));  dCaJSR = Jtr - Jrel;
